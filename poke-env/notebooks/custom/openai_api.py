...
class _AsyncPlayer(Generic[ObsType, ActType], Player):
    ...
    async def _env_move(self, battle: AbstractBattle) -> BattleOrder:
        ...
        print("Turn: ",battle._turn,"putting observations in...")
        await self.observations.async_put(battle_to_send)
        self.obstracker = battle._turn
        print("Turn: ",battle._turn, "obs put in.")
        print("Turn: ",battle._turn, "getting 1 action out to move...")
        action = await self.actions.async_get()
        print("Turn: ",battle._turn, "got an action out", action)
        if action == -1:
...
class OpenAIGymEnv(
    ...
    def step(
        ...
        print("TRAINER: putting action in...", "For turn", battle._turn)
        self._actions.put(action)
        print("TRAINER: action put in.", "For turn", battle._turn)
        print("TRAINER: getting observation out...", "For turn", battle._turn)
        observation = self._observations.get()
        print("TRAINER: observation got out.", "For turn", battle._turn)
        print("TRAINER: observation tracker is at: ", self.agent.obstracker)
